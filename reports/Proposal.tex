...
First of all, we will implement NMF method for topic extraction from documents and evaluate effect of anchor words on accuracy of finding similarity in documents.\\

The results from the previous step will be compared against traditional approaches like LDA used for learning topic models for performance and accuracy and possibly validate author's claim of NMF being better than LDA for topic models. We will also check whether anchor words assumption holds on real life datasets.\\
...
\item Results of experiments and comparison against traditional approaches
\end{itemize}
\item Presentation explaining various aspects of the project and how we went about achieving the aims of Project
\item Source code of algorithms used in the project
\end{enumerate}

\section{Issues}
Finding out anchor words and whether anchor words assumption on real life will be non trivial task. We plan to implement NMF for topic model and will empirically check for existence of anchor words. \\

Depending on nature of distribution of topics for a document we will have to find the distance measure which works best for this distribution. We plan to empirically evaluate quality of results from different distance measures and understand which distance works best for topic models.
