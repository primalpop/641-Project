...
The algorithm has two steps, anchor selection which identifies the anchor words and recovery which recovers the word topic matrix using anchor words and co-occurence matrix. The input to the algorithm is the document corpus D, number of anchor words and tolerance parameters which controls the tradeoff between accuracy and convergence time. The output is word-topic distributions and topic-topic correlation matrix. The steps in the algorithm are as follows
\begin{enumerate}
\item Construct row normalized word co-occurence matrix $\bar{Q}$
\item Find out the anchor words based on $\bar{Q}$ and number of anchor words
\item Recover the word topic matrix using $Q$ and the set of anchor words
\end{enumerate}
The steps in the algorithm are explained in detail in the following sections.


\subsection{Calculating word-by-word co-occurance matrix}

Word by Word co-occurance matrix as we get very noisy version of data and it has property that  $\hat{M} \hat{M}^T -> M M^T $ and $W W^T -> R$ as number of documents increases. For computation of this matrix we counted number of times each word appears in certain document and formed a document word matrix. We represented this matrix $\bar{Hd}$ as sparse as it had very few number of non zero entries. We normalised all the entries by dividing each count by sqrt(n*n-1) where n is total number of words in that document. We also computed $\hat{H}$ which is diagonal matrix which contains elements total count divided by (n*n-1). Then word word covariance matrix Q is given by equation (1).

\begin{equation}
Q = \bar{H} \bar{H}^T - \hat{H}
\end{equation}
The size of this matrix is very large of order of square of size of dictionary. Even though matrix is sparse it will take a huge amount of space. We solve this problem using random projection of matrices. Random projection is based on Johnson-Lindenstrauss lemma which is "If points in a vector space are projected onto a randomly selected subspace of suitably high dimension, then the distances between the points are approximately preserved." as stated in \cite{rp} Random projection reduces size of matrix considerably by projecting it to lower dimension, but keeps the distances between two points approximately same as it was in higher dimension.

\subsection{Recovering Word Topic Matrix}
The algorithm we are implementing is based on a probabilistic approach where we find the conditional probabilities of words given topics. This is based on Bayes' rule of probability. For this purpose we use the row-normalized word co-occurence matrix $\bar{Q}$ which gives the conditional probability among pairs of words i.e $Q_{i,j} = p(w_{1}=i, w_{2}=j)$. So given $\hat{Q_{i}}$, which is a row of the empirical row normalized co-occurrence matrix we wish to find out the $p(z_{1} | w_{1}=i)$ which can best recover the rows as a convex combination of the anchor words. This step can be done in parallel using exponeniated gradient algorithm. The word topic matrix can be recovered once we have $p(z_{1}|w_{1})$ using Baye's rule.
...
\bibitem{rp} \textit{Experiments with random projection}. Dasgupta, Sanjoy. Proceedings of the Sixteenth conference on Uncertainty in artificial intelligence (2000).
